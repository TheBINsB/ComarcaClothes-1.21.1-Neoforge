...
public class CosmeticHeadRenderer implements ICurioRenderer {
    ...
    public CosmeticHeadRenderer(EntityModelSet modelSet) {
        ...
        register(ModClothes.Cono.get(), new cono<>(modelSet.bakeLayer(cono.LAYER_LOCATION)), "cono");
        register(ModClothes.Scara.get(), new scara<>(modelSet.bakeLayer(scara.LAYER_LOCATION)), "scara");
        register(ModClothes.Teemohat.get(), new teemohat<>(modelSet.bakeLayer(teemohat.LAYER_LOCATION)), "teemohat");
        register(ModClothes.Ornntrain.get(), new ornntrain<>(modelSet.bakeLayer(ornntrain.LAYER_LOCATION)), "ornntrain");
        register(ModClothes.Cigarro.get(), new cigarro<>(modelSet.bakeLayer(cigarro.LAYER_LOCATION)), "cigarro");
        register(ModClothes.Chefhat.get(), new chefhat<>(modelSet.bakeLayer(chefhat.LAYER_LOCATION)), "chef");
        register(ModClothes.Caja.get(), new caja<>(modelSet.bakeLayer(caja.LAYER_LOCATION)), "caja");
        register(ModClothes.Caja2.get(), new caja2<>(modelSet.bakeLayer(caja2.LAYER_LOCATION)), "caja");
        register(ModClothes.Caja3.get(), new caja3<>(modelSet.bakeLayer(caja3.LAYER_LOCATION)), "caja3");
        register(ModClothes.Crimson.get(), new crimson<>(modelSet.bakeLayer(crimson.LAYER_LOCATION)), "crimson");
        register(ModClothes.Warped.get(), new warped<>(modelSet.bakeLayer(warped.LAYER_LOCATION)), "warp");
        register(ModClothes.Mushroom.get(), new mushroom<>(modelSet.bakeLayer(mushroom.LAYER_LOCATION)), "mushroom");
    }
    ...
    public <T extends LivingEntity, M extends EntityModel<T>> void render(
        ...
        // Ya no es necesaria pero por si acaso se queda aqui
        //ICurioRenderer.translateIfSneaking(matrixStack, slotContext.entity());

        // Nuevo metodo para seguir el movimiento de la cabeza del jugador
